...
columns = ['Domain', 'Unique Similar Site', 'fetched_at', 'title', 'category', 'category_rank', 'total_visits']
...
def fetch_additional_data(domain, depth=1):
    global unique_similar_sites_df  # Declare the variable as global
    ...
    title = soup.find('div', class_='SiteHeader__Domain-sc-1ybnx66-6 bUzdwD').text.strip()
    category = soup.find('div', class_='StatisticsCategoriesDistribution__CategoryWrapper-fnuckk-4 eKvgan').text.strip()
    category_rank = soup.find('div', class_='SiteHeader__MetricValue-sc-1ybnx66-14 cLauOv').text.strip()
    ...
    if len(unique_similar_sites_df) % 100 == 0:
        save_path = f'Unique_Similar_Sites_{len(unique_similar_sites_df)}.csv'
...
final_save_path = 'Unique_Similar_Sites_Final.csv'  # Adjust the output file path
